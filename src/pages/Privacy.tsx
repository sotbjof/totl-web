...
/**
 * Privacy page displaying Termly-embedded cookie policy
 * Route: /privacy
 */
  ...
  useEffect(() => {
    ...
    const loadTermlyScript = () => {
      ...
      if ((window as any).Termly) {
        console.log('[Privacy] Termly is already available');
        return;
      }
      ...
      if (existingScript && !scriptLoadedRef.current) {
        console.log('[Privacy] Script element exists but Termly not available yet, waiting...');
        // Wait for script to load
        const checkTermly = setInterval(() => {
          if ((window as any).Termly) {
            console.log('[Privacy] Termly is now available');
        ...
        setTimeout(() => {
          clearInterval(checkTermly);
          if (!(window as any).Termly) {
            console.warn('[Privacy] Termly did not load after 5s, removing old script and retrying...');
      ...
      console.log('[Privacy] Loading Termly script...');
      ...
      script.onload = () => {
        console.log('[Privacy] Termly script loaded successfully');
        // Termly will automatically find and populate divs with name="termly-embed"
        // Give it a moment to initialize
        setTimeout(() => {
          if ((window as any).Termly) {
            console.log('[Privacy] Termly initialized, should populate div');
          } else {
            console.warn('[Privacy] Termly script loaded but Termly object not available');
          }
        }, 1000);
      };
      
      script.onerror = () => {
        console.error('[Privacy] Failed to load Termly script');
        scriptLoadedRef.current = false;
      };
  ...
  return (
    <div className="min-h-screen bg-[#f5f7f6] p-4">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold mb-6 text-[#1C8376]">Privacy & Cookie Policy</h1>
